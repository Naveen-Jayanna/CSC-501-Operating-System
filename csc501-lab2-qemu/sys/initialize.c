...
int	nextlock; 					/* variable for holding the nextlock value	*/
#ifdef	Ntty
...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	/* initialize the previous and acquired locks for the process to 0 */
	for (i=0; i<NLOCKS; i++){
		pptr->plocksacquired[i]= 0;	
		pptr->pprevlock[i]= 0;
	}
	currpid = NULLPROC;
	...
	linit();			/* call linit to initialize the locks values */
