...
#define schedprio(process)	(process->pinh!=0?process->pinh:process->pprio)
...
struct	pentry	{
	...
	int	pwaitret;
	int plock;					/* lock for which the process is waiting for	*/
	int	plocksacquired[50];		/* list of locks acquired by the process	*/
	int	pprevlock[50];			/* list of lock previously acquried which should not be reset in the current process execution */
	int pinh;					/* new priority of the process which is inherited 	*/
};
