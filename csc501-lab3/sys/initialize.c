...
fr_map_t frame_tab[NFRAMES];	/* frame table	*/
#ifdef	Ntty
...
struct 	fifo_node * fifohead;	/* fifo queue head			*/
int 	sc_queue_pages;			/* sc queue	pages	*/
int 	sc_index;		/* SC circular queue index */
char 	vers[80];
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	for(i=0; i < 4; i++){
		get_frm(&frm);
		frame_tab[frm].fr_type = FR_TBL;
		frame_tab[frm].fr_status = FRM_MAPPED;
		frame_tab[frm].fr_pid = NULLPROC;
		frame_tab[frm].fr_refcnt = NPTE;
	...
	/* Helper functions*/
	allocate_page_dir(NULLPROC, &pdfrm);
	initialize_page_dir(pdfrm);
